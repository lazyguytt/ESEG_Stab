...
class Config:
    def __init__(self):
        ...
        self.cache_dir      = "cache_dir/EGNN+ESM2"
        self.output_dir     = "cache_dir/EGNN+ESM2/output_1/training"
        self.checkpoint_dir = "cache_dir/EGNN+ESM2/cheakpoints"
        ...
        # ======== ESM 相关配置（★ 新增）========
        self.use_esm       = True          # 打开 ESM 增强
        self.esm_model_name = "esm2_t33_650M_UR50D"
        self.esm_layer      = -1           # -1 = 最后一层
        self.esm_proj_dim   = 64           # ESM 降维后拼到节点上的维度
        self.freeze_esm     = True         # 先冻结 ESM，当特征提取器
...
def evaluate(model, loader, device):
    ...
    for batch in loader:
        ...
        seq = batch.get("seq", None)    # ★ 新增：序列列表 List[str]
        out = model(wt, mt, seq=seq).squeeze(-1)   # ★ 传 seq 给模型
...
def train(cfg: Config):
    ...
    model = EGNNStab(
        ...
        esm_proj_dim=cfg.esm_proj_dim,
        freeze_esm=cfg.freeze_esm,
    ).to(device)
    ...
    for epoch in range(1, cfg.max_epochs + 1):
        ...
        for step, batch in enumerate(
            ...
            mt = {k: (v.to(device) if torch.is_tensor(v) else v) for k, v in batch["mt"].items()}
            y  = batch["ddg"].to(device).float()
            seq = batch.get("seq", None)   # ★ 新增：List[str]
            ...
            with ctx:
                # ★ 注意：这里两个方向调用都要传 seq
                out_dir = model(wt, mt, seq=seq).squeeze(-1)
                out_rev = model(mt, wt, seq=seq).squeeze(-1)

                loss_data = 0.5 * (
                    F.huber_loss(out_dir, y, delta=cfg.huber_delta) +
                    F.huber_loss(out_rev, -y, delta=cfg.huber_delta)
                )
    ...
    # ===== Final Visualization =====
    best_ckpt = torch.load(cfg.best_model_path, map_location=device)
    model.load_state_dict(best_ckpt["model_state_dict"])

    best_val = evaluate(model, val_loader, device)

    visualize_training_only(history, best_val, cfg.output_dir, config=cfg)
    visualize_train_or_test_results(best_val, cfg.output_dir)

    print("Training finished. Best model:", cfg.best_model_path)
    print("Best epoch:", best_ckpt.get("epoch", "?"), "Best RMSE:", best_ckpt.get("best_rmse"))

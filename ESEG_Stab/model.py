...
class EGNNStab(nn.Module):
    ...
    def __init__(self,  d_node,  d_edge, depth=3, d_hidden=160,  nhead_local=3,  dropout=0.2,
                 update_coords=True,  use_ffn=True,  rbf_bins=16,  seqsep_bins=4, orient_dims=0,
                 rbf_centers=None, extra_local_bias=0, hbond_channels=None, drop_hbond=False, use_esm=True,
                 esm_model_name="esm2_t33_650M_UR50D",  esm_layer=-1, esm_proj_dim=64,freeze_esm=True):
        ...
        self.esm_proj_dim = int(esm_proj_dim)
        self.freeze_esm = bool(freeze_esm)

        if self.use_esm:
            self.esm_model, self.esm_alphabet = getattr(esm.pretrained, esm_model_name)()
            self.esm_layer = self.esm_model.num_layers if esm_layer == -1 else int(esm_layer)
            ...
            self.esm_proj = nn.Linear(self.esm_embed_dim, self.esm_proj_dim)

            in_dim = d_node + self.esm_proj_dim
        else:
            self.esm_model = None
        ...
        self.input_proj = nn.Linear(in_dim, 48)
        self.d_model = 48
    ...
    def _build_esm_node_features(self, seq_list, ptr, device):
        if (not self.use_esm) or (self.esm_model is None):
            return None

        assert isinstance(seq_list, (list, tuple)), "seq_list 应该是 List[str]"
        B = len(seq_list)
        assert ptr.numel() == B + 1, f"ptr.shape[0]={ptr.numel()} 和 batch size B={B} 不匹配"

        data = [("seq_%d" % i, s) for i, s in enumerate(seq_list)]
        _, _, tokens = self.esm_batch_converter(data)    # (B, L_tokens)
        tokens = tokens.to(device=device)

        grad_flag = (not self.freeze_esm)
        with torch.set_grad_enabled(grad_flag):
            ...
            reps = out["representations"][self.esm_layer]    # (B, L_tokens, D_esm)

        reps = reps[:, 1:-1, :]   # (B, L_residue, D_esm)

        N_total = int(ptr[-1].item())
        esm_all = reps.new_zeros(N_total, self.esm_embed_dim)

        for i in range(B):
            seq_i = seq_list[i]
            li = reps[i].size(0)                    # 序列长度
            ni = int(ptr[i + 1].item() - ptr[i].item())   # 该图节点数
            ...
            esm_all[start:end] = reps[i, :L, :]

        # 降维
        esm_all_proj = self.esm_proj(esm_all)   # (N_total, esm_proj_dim)
        return esm_all_proj
    ...
    def encode(self, graph, aux, esm_node_feats=None):
        """
        只做编码，返回所有节点的最终特征:
        ...
        """
        x = graph["coords"]                    # (N, 3)
        base_h = graph["node_feats"]           # (N, d_node)

        if self.use_esm and (esm_node_feats is not None):
            assert esm_node_feats.shape[0] == base_h.shape[0], \
                f"ESM 节点数 {esm_node_feats.shape[0]} 与 graph 节点数 {base_h.shape[0]} 不一致"
            h_in = torch.cat([base_h, esm_node_feats.to(base_h.device, base_h.dtype)], dim=-1)
        else:
            h_in = base_h

        h = self.input_proj(h_in)              # (N, 48)
        ei = graph["edge_index"]
    ...
    def forward(self, wt, mt, seq=None, aux_wt=None, aux_mt=None):
        """
        前向:
        ...
        # ===== ESM 节点特征（只用 WT 的序列；如果以后想给 MT 单独一份，可以多传一个 mt_seq）=====
        esm_node = None
        if self.use_esm:
            assert seq is not None, "use_esm=True 时，需要在 forward 传入 seq=List[str]"
            device = wt["node_feats"].device
            esm_node = self._build_esm_node_features(seq_list=seq, ptr=wt["ptr"], device=device)

        h_wt = self.encode(wt, aux_wt, esm_node_feats=esm_node)   # (N_total, d)
        h_mt = self.encode(mt, aux_mt, esm_node_feats=esm_node)   # (N_total, d)

...
def build_wt_mt_graph( wild_path, mutant_path, mutation_str, local_density_radius=8.0,include_edge_plddt=True, 
    ...
    Xn_w, _ = build_node_feature(
        sequence=seq_w_sub, residue_ids=rids_w,
        ca=ca_w, n=n_w, c=c_w, sc_center=sc_w,
        edge_index=ei_w, b_factors=b_w,
        asa_map=asa_wt, rsa_map=rsa_wt, ss_map=ss_wt,
        phi_map=phi_wt, psi_map=psi_wt,
        include_node_hbond=False, hbond_map=hb_wt,
        directed_pairs=None,
        local_density_radius=local_density_radius,
        use_plddt=True
    )
    Xe_w, _ = build_edge_feature(
        edge_index=ei_w, ca=ca_w, residue_ids=rids_w, residue_nums=rnums_w,
        include_edge_hbond=include_edge_hbond,
        hbond_map=hb_wt,
        hbond_min_strength=hbond_min_strength,
        hbond_undirected=hbond_undirected,
        rbf_bins=16, rbf_dmin=2.0, rbf_dmax=22.0,
        seq_sep_max=20,
    ...
    Xn_m, _ = build_node_feature(
        sequence=seq_m_sub, residue_ids=rids_m,
        ca=ca_m, n=n_m, c=c_m, sc_center=sc_m,
        edge_index=ei_m, b_factors=b_m,
        asa_map=asa_mt, rsa_map=rsa_mt, ss_map=ss_mt,
        phi_map=phi_mt, psi_map=psi_mt,
        include_node_hbond=False, hbond_map=hb_mt,
        directed_pairs=None, 
        local_density_radius=local_density_radius,
        use_plddt=True  
    )
    Xe_m, _ = build_edge_feature(
        edge_index=ei_m, ca=ca_m, residue_ids=rids_m, residue_nums=rnums_m,
        include_edge_hbond=include_edge_hbond,   
        hbond_map=hb_mt,
        hbond_min_strength=hbond_min_strength,  
        hbond_undirected=hbond_undirected,       
...
class ProteinDataset(Dataset):
    ...
    def collate_fn(batch) :
        ...
        return {"wt": wt_big, "mt": mt_big, "ddg": ddg}
